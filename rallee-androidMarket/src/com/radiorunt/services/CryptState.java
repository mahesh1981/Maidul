...
public class CryptState {
	...
	public synchronized void setKeys(final byte[] rkey, final byte[] eiv,
			final byte[] div) {
		initialized = false;
		try {
	...
	private synchronized void ocbDecrypt(final byte[] encrypted,
			final byte[] plain, final byte[] nonce, final byte[] tag)
	...
	private synchronized void ocbEncrypt(final byte[] plain,
			final int plain_length, final byte[] encrypted, final byte[] nonce,
